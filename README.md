...
- **One shared implementation.** The datum detection, per-interval slope, distance-windowed central-difference slope and OLS side-slope live in `slope_utils.py` (pure Python, headless-tested); the KP Mouse profile, Depth Profile tool, KP Range Depth + Slope Summary, Workbench rules engine and Burial Planner all call it.
- **Sampling.** Rasters are sampled per station with nearest-cell provider sampling (band 1, best-resolution raster first where several overlap). Contour bathymetry is evaluated where contours actually **cross** the line, with linear interpolation between crossings (major/minor layers merged, duplicate crossings collapsed). Slope windows: per sampling interval in the Depth Profile and KP Range tools (the interval is the analysis scale); the KP Mouse live profile floors its station spacing at half the finest raster cell and measures slope over ± max(cell size, station spacing), so short range lines over coarse grids read the real gradient rather than a nearest-cell staircase; Workbench Assessment uses a central difference over ± its analysis step. The Burial Planner derives local longitudinal slope from its denser persisted profile (a ± profile-step central difference), so a coarse rule-search step does not flatten short, steep terrain. A Burial slope rule can instead set a **slope evaluation length** to average across the burial vehicle's bearing length. Acquisition and 0.1 m boundary refinement use the same selected scale.
