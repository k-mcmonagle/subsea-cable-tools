...
- **Burial Planner** *(beta)* — a guided workflow dock that turns an RPL plus survey data into a burial plan: an ordered stack of exclusion criteria (water depth / slope thresholds with signed direction-aware limits and optional water-depth-banded limits, crossings/proximity with per-feature buffers, seabed-soils polygon classes, KP-range tables, manual ranges — each carrying its criterion class and source reference) is resolved into Exclusion Areas, Constraint Influence Zones, Screening annotations, Insufficient-Information ranges and candidate burial sections with auto-placed start/end events (PLDN/PLUP in plough mode, JET_START/JET_STOP for ROV jet). The plan is always visible in three synchronised views — map layers, a longitudinal profile pane with draggable event markers, and event/section tables with confirm/lock/split/merge and operating-envelope conclusion workflows. Every change lands in an append-only log with exact rollback; events, sections and the input register export to CSV (KP to 3 decimal places with lat/lon and a metadata header), client burial proposals can be imported and diffed against each generation, and rule sets travel as JSON or copy losslessly from Workbench assessments. Analysis runs in the background over the scoped KP window with per-rule caching and 1 m boundary refinement. **Beta — sanity-check the output against your own data and established methods before using it for any operational or engineering decision.** No criteria values are shipped; all limits are user-entered with a source-reference field.
...
- **Cross-profile on request.** The Depth Profile tool and KP Range Depth + Slope Summary can additionally compute **side slope**: an ordinary-least-squares fit of depth vs signed cross-track distance over a configurable port/starboard search transect, perpendicular to the local route tangent.
