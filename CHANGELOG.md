...
### Added
- **Catenary Calculator V2 — multi-span seabed Drape Check** (new "Drape Check & Query" section). A second, independent static solver (`catenary/drape_solver.py`: lumped-node dynamic relaxation with kinetic damping, tension-only axial springs driven to the inextensible limit by rest-length correction, penalty seabed contact, Coulomb stick-slip friction) resolves the **full drape of the cable over the seabed profile**, including resting on high spots the single-span model can only flag as penetration, free spans bridging depressions, an anchored or free bottom end, and seabed friction (μ input). Reports each free span (extent, length, max clearance, min bend radius, max tension), top/anchor tensions, and overlays the draped shape on the plot (dashed). Validated against closed-form catenary results (top tension within 1 %, touchdown within 2.5 %, frictionless flat bed transfers H to the anchor within 2 %) in `tests/test_drape_solver.py`. Friction equilibria are lay-history dependent and reported as indicative. See `catenary/MODEL_NOTES.md`.
- **Catenary Calculator V2 — point query system.** "Query at s from chute" reports tension, angle, bend radius, depth, seabed clearance and contact state at any arc position along the cable — from the drape result when one has been run (including the on-bed section), otherwise from the single-span solution.
