...
def build_report_html(plan: Dict,
    ...
    if path_result and path_result.get("tool_path_wkt"):
        ...
        path_meta = [
            ...
            ("Course changes not met exactly (best fit / review)",
             str(path_summary.get("best_fit_count") or 0)),
            ("Controls in downstream recovery",
             str(path_summary.get("recovery_control_count") or 0)),
            ("Wide endpoint recovery",
             "yes — review route scope"
             if path_summary.get("wide_recovery") else "no"),
            ("Deviation-threshold reviews",
             str(path_summary.get("deviation_review_count") or 0)),
